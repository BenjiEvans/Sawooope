...
public class Sheep extends Creature {
	 ...
	 * to see if this sheep should be moved
	 */
	private void listenForMove(){
		
		if(currState != IDLE) setStateTo(IDLE);
			if(moves[NORTH]){
	...
	public void move(int direction) {
		...
		if(currState == WALKING){
			...
			{
				if(direction == NORTH && facingDir == SOUTH)moveBack(direction);
				else if(direction == SOUTH && facingDir == NORTH)moveBack(direction);
				else if(direction == EAST && facingDir == WEST)moveBack(direction);
				else if(direction == WEST && facingDir == EAST)moveBack(direction); 
