...
/**
 * LevelInputProcessor keeps track of all touch events 
 * and detects what kind of gestures the users is making 
 * on the screen( for example: swiping )
 * 
 * @author Benji
 *
 */
public class LevelInputProcessor{
	...
	/**
	 * 
	 * @param x x location where the screen is pressed
	 * @param y y location wheere the screen is pressed
	 */
	...
	/**
	 * Checks to see if the object was pressed 
	 * and stores it if it is pressed 
	 * @param object some board object
	...
	/**
	 * 
	 * @param x x location where user dragged to 
	 * @param y y location where user dragged to 
	 */
	...
	/**
	 * 
	 * @param x x location of where user released screen
	 * @param y y location of where user released screen
	 */
	...
	/**
	 * Gets the world coordinate(position in game)
	 ...
	 * Level's map (map position) and is used by the LevelInputProcesser to detect gestures on the 
	 * screen.
	 * @author Benji
	 */
	public class TouchPosition{
		...
		/**
		 * 
		 * @param x x location where touch event occurred
		 * @param y y location where touch event occurred
		 */
		 ...
		 * @param pos position
		 * @return true if the TouchPosition's position on the map 
		 *  is within the radius of the TouchPosition
		 */
		...
		/**
		 * 
		 * @return the x coordinate of where the 
		...
		/**
		 * 
		 * @return the y coordinate of where the 
		 * the screen was touched
		 */
		...
		/**
		 * Determines if to TouchPositions are the same (determined by their map position)
		 * @param pos touch position being compared
	 ...
	 * Object-Oriented design, However... it will have to do for now.
	 * @author Benji
	 *
	 */
	public class TouchAction implements Touchable{
		...
		/**
		 * 
		 * @param beginTime time stamp of when the touch event started
		 * @param endTime time stamp if when the touch event ended
		 */
		TouchAction(long beginTime, long endTime){
			...
			if(elapsed < SWIPE_TIME){
				...
				int direction = getSwipeDirection(pressed.getScreenx(), pressed.getScreeny(), release.getScreenx(), release.getScreeny());
		...
		/**
		 * Gets the position on the TileMap that was tapped 
		 * if a tap event occurred
